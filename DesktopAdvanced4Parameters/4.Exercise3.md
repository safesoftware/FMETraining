...
<br><br>The point is that now the MapInfo and CSV writers share a parameter defining their output folder. If we wish to change where they are being written to (say when the path changes from FMEData2018 to FMEData2018) we only need to edit the private parameter to fix both writers. That's why we did what we did in step #4.
