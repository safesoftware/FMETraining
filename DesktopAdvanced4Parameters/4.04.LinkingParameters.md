...
Because the FME parameter is now linked to the user parameter, whatever value the user sets for that user parameter will be applied directly to the FME Parameter (Features Per Transaction):
...
Creating a linked FME parameter directly has an obvious advantage that it is a single-step process. The creation and linking of the user parameter is done in a single action.
