...
Since ultimately we want to report on 3-1-1 calls by local planning area, let's see if any of our features are missing values for the `Local_Area` attribute. To do so, scroll over in your table until you see the `Local_Area` attribute. Then, right-click and select **Sort Alphabetical Ascending**:
