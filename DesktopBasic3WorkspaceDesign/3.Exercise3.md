...
<br>**9) Remove Sampler**
<br>To complete the project let's run the workspace on the full dataset. You can do this either by deleting the Sampler transformer (the simplest solution) or disabling it and adding a connection to bypass it.

To bypass the Sampler, first draw a second connection from the source data to the first PointOnAreaOverlayer's Point input port:
...
Because this is the first time using the full dataset, we might consider caching this data too. However, that would make the workspace run much slower. So, for reasons of time and performance, turn off the Run with Caching option and then re-run the translation.
