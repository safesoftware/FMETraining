...
The Log Debug option turns on a series of extra log messages that are usually hidden from the user. Not only will a lot of the underlying mapping file be exposed, there will also be a number of ERROR messages labeled BADNEWS.
...
In the image above the author is only viewing the error messages. Which makes it easy to see what is causing their translation to fail. 
