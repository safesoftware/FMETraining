...
<br>**8) Filter Data**
<br>Let's filter our data in the Data Inspector to control what is displayed. We'd like to keep all of the neighborhood layer, but only show industrially-zoned zoning districts.
...
The Logic parameter will automatically be set to "OR", meaning each feature must **either** have an industrial zone category, **or** have a value for NeighborhoodID. For other tests you can change the logic (AND, OR, or NOT in all their permutations).
