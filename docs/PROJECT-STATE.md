...
| **KNOW-2362** release flow needs `git` in the app image (silent empty `scan_saved_lessons`) | — | — | **In Backlog (box-readiness #1)** | — | `scan_saved_lessons` shells to `git`, absent from the image → `except: pass` → empty release list. Decide: add git+`safe.directory` to the image, or **DB-native detection** via `report_lesson_drafts.mark_saved` (preferred). Surfaced QA of #53. |
| **KNOW-2359** in-app publishing needs a content corpus on the box | — | — | **In Backlog (box-readiness #2 — decision)** | — | Box `LESSON_CONTENT_ROOT` unset → `/opt/fme-train` (no corpus, not a git tree). Decide: provision a writable git corpus on the box vs. keep publishing a local-checkout workflow vs. v2/MCP (KNOW-2307). Needs box access (KNOW-2309, IT-blocked). |
| **KNOW-2361** local dev can't rehost images (minio creds + no S3 endpoint) | — | — | **In Backlog (box-readiness #3)** | — | Compose pins `minioadmin`; upload path ignores `S3_ENDPOINT_URL`. Decide: thread the endpoint (minio) vs. stop pinning fake creds. Box side = R2 (real `AWS_*` in `/etc/fme-train/env`). |
...
**▶ NOW — box-readiness (make publishing work on the box), in order:**
1. **KNOW-2362** — add `git` to the app image (or switch `scan_saved_lessons` to DB-native detection). *In-repo, codeable now; decision: git-in-image vs DB-detection.*
2. **KNOW-2359** — decide + provision how the box gets a content corpus (writable git tree at `LESSON_CONTENT_ROOT`) vs. keep publishing local-only vs. v2/MCP. *The gating decision; needs box access (KNOW-2309, IT-blocked).*
3. **KNOW-2361** — local-dev S3 image creds (thread `S3_ENDPOINT_URL` / stop pinning minio) + box side = real `AWS_*` in `/etc/fme-train/env` (R2).

**Then:** KNOW-2358 WS-E (locks/history); the KNOW-2323 Releases-UX redesign (incl. the draft-first + warn-on-live safety AC); KNOW-2307 (v2/MCP); `lesson_drafts` reconciliation; mapping→DB; KNOW-2313 (report XSS); retire the legacy launcher (KNOW-2351, now unblocked by #53 reaching parity — pending box-readiness).
