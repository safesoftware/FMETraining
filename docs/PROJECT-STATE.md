...
| **KNOW-2364** remove the 7830-file git corpus + default dev to S3 | — | — | **In Backlog** (follow-up to 2307) | — | Removal **proven safe** (corpus moved aside, gates green). Lands as a dedicated PR. |
...
**▶ NOW — finish box-readiness publishing (small):**
1. **Deploy PR #55 + set the box env:** `CONTENT_SOURCE=s3mirror` (+ `CONTENT_S3_BASE_URL`) and the publish creds (`SKILLJAR_*`/`AWS_*`) in `/etc/fme-train/env` → publishing works on the box. (Needs box access — KNOW-2309, IT-blocked.)
2. **KNOW-2361** — local-dev S3 image-upload creds (thread `S3_ENDPOINT_URL` / stop pinning minio); box side = real `AWS_*` (R2).
3. **KNOW-2364** — remove the 7830-file git corpus + default local dev to S3 (proven safe; dedicated PR).

**Then:** KNOW-2358 WS-E (locks/history); the KNOW-2323 Releases-UX redesign (draft-first + warn-on-live safety AC); `lesson_drafts` reconciliation; mapping→DB / R3 (`data/` writability); KNOW-2313 (report XSS); retire the legacy launcher (KNOW-2351).
