...
**✅ DONE — dev-infra + security (2026-06-16):**
- **KNOW-2361** (PR #58) — stopped pinning `minioadmin`; real `.env` creds flow + dead minio service removed. *Dev-convenience; box unaffected.*
- **KNOW-2366** (PR #59) — **security:** whole app now behind login (default-deny `AuthMiddleware`). Box QA found `/drafts`/`/release`/report HTML readable logged-out. Deployed + QA-passed on the box. **Open follow-ups:** rotate the leaked `AWS_ACCESS_KEY_ID`; evaluate an IP allowlist in front of the box (defense in depth).

**▶ NOW — next:** **KNOW-2358 WS-E** (release locks/history) **or** the **KNOW-2323 Releases-UX redesign** (draft-first + warn-on-live safety AC) — the natural follow-ons now that publishing + auth are solid.

**Then:** `lesson_drafts` reconciliation; mapping→DB / R3 (`data/` writability); KNOW-2313 (report XSS); retire the legacy launcher (KNOW-2351 — now largely unblocked: the publish flow + box are live; only `serve.py`'s single-lesson push remains legacy).
