...
| Local dev | Compose (`make up`) | FastAPI + Postgres 16 + worker (minio removed, KNOW-2361) | `InProcessTaskDispatcher`; `LocalFolderSource` for content; real `.env` AWS creds for image rehost. This is where the agent runs functional QA. |
...
| **KNOW-2358** port Skilljar publish/release flow into the app (WS-B/C/D/E) | — (merged) | **PR #53 + #61** | **✅ Closed** | `as-far-as-i-golden-wave.md` | WS-B/C/D shipped in #53 (validated live); **WS-E (locks/history) shipped in PR #61** (`8b1da8ac`) — per-course locks (409 on concurrent same-course release) + audit history + `/api/release-history` + a "Recent releases" panel. Folded in/closed KNOW-2321/2322 + the Step-4 slice of 2323. Requires `--workers 1` (KNOW-2368). **Merged to `main`; box redeploy pending.** |
| **KNOW-2368** box ran 2 uvicorn workers → split in-process release-log/lock state ("No log for key") | — (merged) | **PR #62** (`b4713c68`) | **✅ Closed** | — | `--workers 2` → ~half the release-log polls hit the worker that didn't start the release → 404 + flaky WS-E finalize. Fixed to `--workers 1` in `setup-ec2.sh`; live box unit hand-edited + restarted (persists across deploys). |
| **KNOW-2369** fake-Skilljar local mode for safe publish-flow QA | — | — | **In Backlog** (recommended next) | — | Make `SKILLJAR_API_BASE` env-configurable / add a `SKILLJAR_FAKE` toggle so `make up` runs the whole publish flow (incl. WS-E) end-to-end against canned responses, never hitting `api.skilljar.com`. Unblocks safe local QA now that the box has active users. Parent KNOW-2345. |
...
| **KNOW-2361** local dev can't rehost images (minio creds + no S3 endpoint) | — (merged) | **PR #58** (`5d4d8ee2`) | **✅ Done** | — | Stopped pinning `minioadmin` (real `.env` creds flow → real bucket, matches the box) + removed the dead minio service/volume. |
| **KNOW-2366** auth gap: whole app not behind login (only `/api/*` gated) | — (merged) | **PR #59** (`48d9de2f`) | **✅ Closed** (box QA passed) | — | Flipped `AuthMiddleware` to default-deny: public allowlist `/`,`/_ping`,`/health`,`/auth/*`,`/static/*` only; `/api/*`→401, any other path→302 sign-in. `/drafts`/`/release`/report HTML (lesson content + edit plans) were readable logged-out. New `tests/integration/test_auth_gate.py`; 637/19 green. Deployed + QA-passed on the box. Follow-ups: rotate the leaked `AWS_ACCESS_KEY_ID`; consider an IP allowlist. |
...
**✅ DONE — 2026-06-16/17:**
- **KNOW-2361** (PR #58) — stopped pinning `minioadmin`; real `.env` creds flow + dead minio service removed.
- **KNOW-2366** (PR #59) — **security:** whole app behind login (default-deny `AuthMiddleware`). Deployed + box-QA-passed.
- **KNOW-2358 WS-E** (PR #61) — release locks + audit history (completes KNOW-2358). Merged to `main`; **box redeploy pending**.
- **KNOW-2368** (PR #62) — box → `--workers 1` (in-process release-log/lock state can't be split). Live box unit fixed + restarted.

**⏸ RESUME HERE (Sam paused 2026-06-17):**
1. **Redeploy the box for WS-E:** `sudo -u fmetrain bash /opt/fme-train/bin/deploy-prod.sh main` (the `--workers 1` unit is already in place; `deploy-prod.sh` doesn't touch the unit).
2. **KNOW-2369 — fake-Skilljar local mode** *(recommended first)*: so the publish flow + WS-E are QA-able locally without touching prod Skilljar (the box has active users → live QA releases mutate real Skilljar).
3. **Open follow-ups:** rotate the leaked `AWS_ACCESS_KEY_ID`; evaluate an IP allowlist on the box (defense in depth).

**▶ Then (backlog order):** **KNOW-2323 Releases-UX redesign** (draft-first + warn-on-live safety AC) → `lesson_drafts` reconciliation → mapping→DB / R3 (`data/` writability) → KNOW-2313 (report XSS) → retire the legacy launcher (KNOW-2351 — publish flow + box live; only `serve.py`'s single-lesson push remains legacy).
