...
Connect the output port of the HTMLReportGenerator_2 to the Right input port of the FeatureJoiner. Then drag the feature connection going into the KML writer feature type into the FeatureJoiner Left input port. To drag a connection, mouse over the connection line until it is selected with a blue outline, then click and drag to change its connection. Your cursor should snap to the port you want to connect to:
...
You can expand the contents of a multi-line attribute by clicking the gray ellipsis button next to the cell to expand the value into its own window:
...
Or if you look in the Feature Information window in Data Inspector:
...
Double-click it to set its parameters. We will be setting Color and Fill Color. You can choose whichever colors you prefer. In this example, we will use orange. Click the ellipsis button next to Color and choose the color you want. We used #ff4500, a hexadecimal color code you can enter in the HTML box:
