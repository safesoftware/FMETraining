...
This exercise was made to demonstrate how to stream KML Layers produced from workspaces to an online map. When a user selects a layer, a workspace is activated to produce the KML Layer. However, by default Esri will cache the results of the KML Layer for 30 minutes. Esri caches layers for to enhance the performance. However, if your data needs to be refreshed on a shorter interval this code will have to be modified.  

<!--Warning Section-->

<br><table style="border-spacing: 0px">
<tr>
...
The viewDiv which contains the map is given a height and width of 100% (line 147-148). The map will displayed on the entire screen regardless of the screen size.

The layer toggle which contains the layers available will be formatted similarly to the heading. It will be placed on top of the map and has a white background. However, the it appear in the top right hand corner of the screen.
...
Drag and drop each file into the "Drop file to upload" area, one at a time. Then, click Import.
...
![](./Images/9.6.7.restapipermissions.png)
...
Find and replace all of the localhost with your Public IP Address.
