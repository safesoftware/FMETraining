...
The layers are created from workspaces hosted on the FME Server. We are using a Web Service URL to activate the layers.
...
Open the FME Server by visiting http://localhost/fmeserver. Sign into the account as the admin. The username will be admin and the password will be admin as well. Next, look on the left-hand panel and click Projects.
...
<br>**7) Update the token permissions**
...
![](./Images/9.6.5.token.png)

Click on the token we created earlier in the course. This should be called Rest API Training Course.

![](./Images/9.6.6.restapitoken.png)

Then, find the Repositories tab and find the WebApplication repository. From here, we can select the Download, Read, and Run permissions.

![](./Images/9.6.7.webapplication.png)




<br>**8) Review the require and the function section**
...
<br>**9) Add Web Service URLs for each new KMLLayer Created**
...
**Paste the Web Service URL into the URLs section of the transporationLyr (line 28).**

```
var transportationLyr = new KMLLayer({
          url: "http://localhost/fmedatastreaming/WebApplication/webapp.skytrains.fmw?", // Skytrain Layer
          id: "skytrainlayer",
          visible: false
        });
```
...
**Find the webapp.skytrainbuffer.fmw in the WebApplication repository. Then, copy the Web Service URL from the Advanced menu and paste it into the skytrainbufferLyr URL (line 33).**
...
**Find the webapp.parks.fmw in the WebApplication repository. Then, copy the Web Service URL from the Advanced menu and paste it into the parksLyr URL (line 38).**
...
```
**Find the webapp.foodTrucks.fmw in the WebApplication repository. Then, copy the Web Service URL from the Advanced menu and paste it into the foodLyr URL (line 43).**
...
<br>**10) Update localhost to your Public IP Address in the URLs**
...
<br>**11) Update the basemap used in the map**
...
<br>**12) Add the Layers to map.add**
...
<br>**13) Update the center of the map and the zoom level**
...
```
<br>**14) Review the code to create the toggle variables**
...
<br>**15) Add the Code to toggle the visibility of the food layer**
