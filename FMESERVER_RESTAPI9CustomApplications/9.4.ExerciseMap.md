...
<br> We will be writing a JavaScript file to make our webpage interactive. It is a best practice to separate your JavaScript from the HTML to ensure that each file is as clean and easy to follow as possible.
...
![](./Images/9.4.5.heading.png)
 ...
 Then, we create a form (line 20-27), with buttons to activate different functions in the JavaScript. The draw button will allow the user to draw on the map. The reset button will reset the users drawing. The third button will use the coordinates selected from the user and clip the data to the selected area. 
