...
In the head section (lines 4-11). Here, like the past exercises, there is are connections to the FME Server Example CSS and the FME Server JavaScript.  
...
Next, we can see the session ID that was created from the getSession and test that it is refreshing properly. In the setVars function which is the callback from the getSession function, there is console.log. This will log the JSON that was returned from the FME Server. This JSON will contain a number for the session. If we refresh this page the number should change. This number is used to create a folder in the Server with the Data Uploaded.  
...
Therefore, assign the path to json.serviceResponse.session.
...
On line 82 we need to modify the the code to specify what element we would like to add. In our case we would like to add an input element.
...
Next, we have the uploadFile function. This function will upload the file selected to the FME Server. We need to modify the dataUpload function to include the right parameters.
