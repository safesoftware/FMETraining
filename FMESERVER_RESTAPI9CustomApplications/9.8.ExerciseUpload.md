...
Go to C:/FMEData2019/Resources/RESTAPI/myFourthApp.html then right click on myFourthApp.html and select Edit with Notepad++.  
...
On line 26, there is a button which the user will press to run the workspace with the data uploaded to FME Server.
...
Next, we can see the session ID that was created from the getSession and test that it is refreshing properly. In the setVars function which is the callback from the getSession function, there is console.log (line 62). This will log the JSON that was returned from FME Server. This JSON will contain a number for the session. If we refresh this page the number should change. This number is used to create a folder in the Server with the Data Uploaded.
...
Next, we have the uploadFile function (lines 92-97). This function will upload the file selected to FME Server. We need to modify the dataUpload function to include the right parameters.
...
<br>The processParams function (lines 123-150) gets the parameters from the options form and then sets the variables in a format FME Server will understand.
...
To run the web application go to http://localhost:8000/myFourthApp/myFourthApp.html
