...
Go to C:\FMEData2018\Resources\RESTAPI\myFirstApp\myFirstAppStart.html. Then right click on the file myFirstAppStart.html and select Edit with Notepad++. The file contains an almost complete web application. We will be filling in information to complete it.

<br>**2) Fill in the to the FME Server JavaScript API**
...
The link to the JavaScript API has to be included in all applications that use it. It's also a great resource to use if you would like to know more about the JavaScript API as it lists all the functions.
...
<br>**3) Find the form in the next section and fill in the suggested values**

Scroll down to line 21 and find the form section.
<br>
![](./Images/9.1.3.FormSection.png)


The form is indicated by the tag &lt; form&#62;. We have set the id of the form to "exampleForm". If we needed the JavaScript to act upon the form this  
...
In the form, we will create a label and an input. The label is entitled  
Repository: which will appear on the page. Then we need to give the input an id so it can be called upon later. The input type is text. **We need to edit the value to be "Samples".**

![](./Images/9.1.5.Samples.png)

We repeat the same input process for the workspace. **The value should be updated to "austinDownload.fmw".**

![](./Images/9.1.6.austinDownload.png)

Next, we need to create an input type for the next section of code. We would like the next input type to be button.
...
After the first form has been activated, there needs to be another form to send the data to. Locate line 36 and 37, and replace "//Form id" with "output-form".
...
If we looked at the website now, it would have a form but no functionality. JavaScript adds functionality to the page.

<br>**5) Update the Server and Token**

Go to the start of the of the JavaScript section. This should be on line 39.
...
Next, we can view the process form function. This function uses the FME Server JavaScript API to get the Workspace Parameters. This means that the parameters will be updated dynamically as the user enters their workspace and repository.
...
To use this function we need three parameters, the repository, workspace, and callback.
...
We need to change "Find the repository ID and update it here" to "repository-name". We also need to update
"Find the workspace ID and Update it here" to "workspace-name".
...
The next function is the callback function. A callback function is specified in the previous function. It takes the json returned from the FME Server and uses it.
...
The first part of the code will clear the output form if there is anything in their previously. Then the generateFormItems function is called but currently there are on parameters in the function so it will not work.

![](./Images/9.1.17.generateformitems.png)

The id is the form container to put the elements into. In our case this is the blank form called "output-form". The json is the object containing the form data and in this case ours is just json. The correct parameters should look like this.
...
Next, we need to specify the what the input will be. In our case it should be a button. Line 64 should now read:
...
In the beginning for the loop, we are specifying that the loop will begin at the first form element and continue until the form is completed. The variable element is created with the form elements used in the output form.
...
Next, inspect the drop down menu beside Output Cooridinate System.
