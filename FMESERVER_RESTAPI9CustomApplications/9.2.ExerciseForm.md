...
<td style="border: 1px solid darkorange">C:\FMEData2019\Resources\RESTAPI\myFirstApp\myFirstAppStart.html  </td>
</tr>

<tr>
<td style="border: 1px solid darkorange; font-weight: bold">Completed HTML </td>
<td style="border: 1px solid darkorange">C:\FMEData2019\Resources\RESTAPI\myFirstApp\myFirstApp.html  </td>
</tr>
...
Find the code snippet below and click Edit in JSFiddle. The file contains an almost complete web application but is missing some key information. Follow the steps below to get this app up and running. In the HTML document, any information that requires updating is indicated by // (the JavaScript comment symbol) or &lt;!----> (the HTML comment symbol).

<script async src="//jsfiddle.net/siennaemery/c5n9g70r/25/embed/html,js,result/"></script>

<br>**2) Add the FME Server JavaScript API Link**

![](./Images/9.2.1.JavaScript.png)

_Please note that the screenshots for this course were taken in light mode. If you would like to enable light mode, go to the top right corner of the JSFiddle. Find Settings and disable Dark theme._
...
and paste ```"https://api.fmeserver.com/js/v3/FMEServer.js" ```


The link to the [FME Server JavaScript API](https://api.fmeserver.com/js/v3/FMEServer.js) must be included in all applications that use the FME Server JavaScript API. It's a great resource to use if you would like to learn more about the FME Server JavaScript API because it lists all the available functions.  

<iframe width="100%" height="300" src="//jsfiddle.net/siennaemery/gb2vzm7L/6/embedded/html,js,result/" allowfullscreen="allowfullscreen" allowpaymentrequest frameborder="0"></iframe>
...
In the form, we will create a label and an input. The label is entitled Repository: which will appear on the page. Then as with the form, we need to give the input an id so it can be called upon later. The input type is text. **We need to edit the value on line 24 to be "Samples".**

We will repeat the same input process for the workspace. **The value should be updated to "austinDownload.fmw" on line 28.**
...
**Delete "// Input Type" and then replace it with "button" on line 30.**


Having this button will activate the "processForm();" function. The button will have the value or label of Generate Full Form.

<iframe width="100%" height="300" src="//jsfiddle.net/siennaemery/n36dzLav/28/embedded/html,js,result/" allowfullscreen="allowfullscreen" allowpaymentrequest frameborder="0"></iframe>
...
**Locate line 35, and replace "//Form id" with "output-form".**

<script async src="//jsfiddle.net/siennaemery/6x2adzm9/20/embed/html,js,result/"></script>
...
![](./Images/9.2.3.Form.png)


<br>**5) Update the FME Server URL and Token**

Switch to the JavaScript tab. On lines 3 and 4 we initialize the Server.
...
<script async src="//jsfiddle.net/siennaemery/e038pan6/13/embed/js,html,result/"></script>

<br>**6) Update the processForm function**

![](./Images/9.2.4.ProcessForm.png)

Next, we can view the processForm function this is located on line number 8. This function uses the FME Server JavaScript API to get the Workspace Parameters using the repository and workspace values entered by the user. We use the function getWorkspaceParameters to get the workspace parameters created in the call.
...
<script async src="//jsfiddle.net/siennaemery/bqx8m61c/21/embed/js,html,result/"></script>

<br>**7) Modify the generateForm function to produce a form**

Go to the generateForm function, this is located on line 16. The next function is the callback function. A callback function is specified in the previous function. It takes the json returned from FME Server and uses it.
...
On line 19 find FMEServer.generateFormItems(); and update it to:
...
<iframe width="100%" height="300" src="//jsfiddle.net/siennaemery/pL8h3gxs/13/embedded/js,html,result/" allowfullscreen="allowfullscreen" allowpaymentrequest frameborder="0"></iframe>
...
Next, we need to specify what the input will be. In our case, it should be a button. Line 25 should now read:

    submitButton.type = "button"; // Fill in the type

Next, we can specify what the value should be the value of the button is what text is displayed on the button. This should be descriptive of what the user should do. Line 26 should now read:

    submitButton.value = "Run Data Download"; // Fill in the value

<iframe width="100%" height="300" src="//jsfiddle.net/siennaemery/r7zdk1sg/17/embedded/js,html,result/" allowfullscreen="allowfullscreen" allowpaymentrequest frameborder="0"></iframe>


<br>**9) Review the runDataDownload function**

We are now going to skip the showResults function for now and go to RunDataDownload beginning on line 43.

![](./Images/9.2.9.dataparam.png)
...
Next, we have to fill in the various element types in the form. The element type is the type of form element used, this can include a drop down list, check list or a text box. To find what elements exist within our form.

Before running the script, select the JavaScript dropdown menu and change the LOAD TYPE to No wrap- bottom of the &lt;head>.

![](./Images/9.2.12.loadtype.png)

 Then, click Run. Then, go to the Results panel in the bottom right hand corner of the screen.


Once at the page right click and select inspect.

![](./Images/9.2.12.inspect.png)
...
![](./Images/9.2.13.GenerateForm.png)
...
![](./Images/9.2.15.elementype.png)

Next, inspect the drop down menu beside Output Coordinate System.

![](./Images/9.2.16.Select.png)

Here, we can see the element type is select.

Finally, inspect the text box next to Minimum X.

![](./Images/9.2.17.input.png)

Here, we can see the element type is input.

<br>**11) Update the runDataDownload function**

Go to line 50 and review the for loop.

![](./Images/9.2.18.elem.png)

In line 52 we see the first if statement. This statement should say, if the element type is equal to select. Then add the element name and the selected value to the params variable. At the end of the params variable we are adding an ampersand or & so we can add another parameter.
...
<iframe width="100%" height="300" src="//jsfiddle.net/siennaemery/ytpg6xve/7/embedded/js,html,result/" allowfullscreen="allowfullscreen" allowpaymentrequest frameborder="0"></iframe>
...
![](./Images/9.2.21.Download.png)



![](./Images/9.2.21.DataDownload.png)
...
Next, we need to find the function above the runDataDownload function. This is showResults function and should be found on line 31.

![](./Images/9.2.22.result.png)
...
Select Run then click on GenerateFullForm. Then, click the Run Data Download button.
...
![](./Images/9.2.24.serviceresponse.png)

<br>**13) Update the showResults function with the URL location**

In the showResults function, on line 38 there is an empty variable for the download link.

![](./Images/9.2.25.sresult.png)
...
<iframe width="100%" height="300" src="//jsfiddle.net/siennaemery/zo1hdbtu/9/embedded/js,html,result/" allowfullscreen="allowfullscreen" allowpaymentrequest frameborder="0"></iframe>
...
If you missed a step, the full code can be found at the end of step 13. Just update the token to your own.
