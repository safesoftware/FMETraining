...
<td style="border: 1px solid darkorange">C:\FMEData2016\Workspaces\DesktopAdvanced\CustomTransformers-Ex3-Complete.fmw</td>
</tr>
...
Notice that, currently there are two instances of the custom transformer. Both produce an attribute with the same name (DensityValue). It would be helpful if the user of the custom transformer could define what the name of that attribute should be.

Let’s allow that. Click on the tab labelled DensityEvaluator to switch the canvas to the custom transformer definition. Open the parameters dialog for the ExpressionEvaluator. Next to New Attribute click the down-arrow and choose User Parameter > Create User Parameter:
...
Click OK to close the dialog. Run the workspace to prove the label changed worked, and that the output is still correct.
...
<br>At the moment this workspace is calculating the number of items (here persons) per square kilometre of land. This works for the original scenario, however, other uses of this transformer might find different units to be more useful.
...
<pre>@Value(TotalPopulation2001)/(@Value(NeighborhoodArea)*@Value(WeightingAttribute))</pre>
