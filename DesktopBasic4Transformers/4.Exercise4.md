...
If you expand your attributes for the PostalAddress writer, you will notice that NumberCrimes doesn't appear. When we manually set Event to "Noise Control Update" we changed the Attribute Definition for this writer from Automatic to Manual. This means it will not automaticlaly update to add new attributes created during translation. Therefore we have to either swtich back to Automatic (which would bring along many other unwanted attributes), or simply add a new attribute called NumberCrimes here. Give it type "number" and cell width 6.
