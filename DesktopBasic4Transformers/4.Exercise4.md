...
This request is a significant test of the city's open data policy, and there's no question of not complying. However, a crisis arises as the current datasets for crime (CSV, non-spatial) is not joined to the address database in any way.
...
For the Join Mode select *Inner*. This means that we want only address with crimes to be returned.
...
<br>**8) Add Aggregator**
<br>Interestingly, although 148 addresses enter the FeatureJoiner, 197 emerge from it:
