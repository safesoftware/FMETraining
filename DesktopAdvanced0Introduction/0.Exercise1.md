...
<br><br>That's ok if you don't get it finished completely. The complete instructions are below. Just try your best! 
...
Once the Microsoft Excel reader is added. Add an Inspector transformer to inspect the data. 

***NB:*** *If you inspect your data and you do not see any geometry, ensure that you used an Inspector to inspect your data.*
...
The coordinate system for this dataset is UTM83-10, the other dataset LL84; so one of these datasets must be reprojected to match the other. 
...
<br>Next we will run the translation to check if the buffer amount was enough. Firstly, on the top menu bar select Run &gt; Run with Feature Caching: 
...
It looks like these buffers only cover approximately 1 block, so we should increase their size. Back in FME Workbench, change the Buffer Amount parameter in the Bufferer to 250. 
...
Run the translation again if you wish, and inspect the SpatialFilter:Passed output port (alongside the Bufferer output) to make sure the results are correct. 
...
<br>**10) Save and Run Translation**
