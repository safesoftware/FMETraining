...
<br>Open FME Workbench and click New to create a blank workspace. Add a new Reader, in the Add Reader dialog enter:
<table>
...
<br>Add a KMLStyler transformer between the SpatialFilter:Passed output port and the ArtNearTransit Writer. In the parameters under Icon > Name, click the ellipsis then select the icon J1. Then for Label Style set the Scale to 0 and the Opacity to 0, this will hide the labels. 
