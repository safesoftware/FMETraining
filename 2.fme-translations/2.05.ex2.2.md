...
The local areas currently appear without any context. Let's improve the display by adding a background map. Select Tools > FME Options > Background Map. Click the dropdown next to Background Format:
...
Click the button to turn feature caching on, or go to Run &gt; Run with Feature Caching. Then run your workspace again. You should notice a little green square with a magnifying glass filling up on your reader feature type:

![](.\Images\feature-cache-icon.png)

This is a feature cache; the features at that point of the translation are recorded in this cache and can be inspected. This is useful for two reasons:
