...
<br><br>The point is that now the MapInfo and CSV Writers share a parameter defining their output folder. If we wish to change where they are being written to (say when the path changes from FMEData2017 to FMEData2017) we only need to edit the private parameter to fix both Writers.
