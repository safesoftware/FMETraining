...
By changing this from Default to No, the user is no longer allowed to select an attribute in the Translation Parameters prompt:
...
<br><br>Well - as you saw above - not every FME parameter allows attribute assignment; and making it a user parameter will not change this. For example, the Snapper is never allowed attribute assignment. Having a Yes/No option for a user parameter linked to it is pointless, because Yes is never allowed. That's why we use the term "Default".
