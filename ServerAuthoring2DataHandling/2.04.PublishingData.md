...
When the source data for a translation is stored as files (rather than a feed or database), it is best practice to publish your data to the Resources folder separate from the workspace. It is also possible to publish data to an FME Server repository along with the workspace.
