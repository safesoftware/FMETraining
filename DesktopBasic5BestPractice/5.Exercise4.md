...
In FME Workbench, add a PostGIS reader, then for the Connection, click the drop-down and select Add Database Connection. Then in the Add Database Connection dialog, enter the connection parameters obtained through the web browser:

![](./Images/Img5.220.Ex4.DatabaseConnectionDialog.png)

Give the connection a name and click Save. Click on Parameters to select the table we want to read from the database, select the public.CompleteRoads table. Then click OK twice to add the reader to the canvas.
...
<br>Regardless of our expression, there's another last snag to cover... one-way streets. Currently, we have no solution in place to prevent us from driving the wrong direction.
...
There is at least one change (above) caused by a route that previously traveled the wrong-way (west-east) along a street that is one way (east-west)!
