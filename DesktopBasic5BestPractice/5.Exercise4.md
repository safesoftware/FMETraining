...
Now we're ready to go. Re-run the workspace. You could inspect the ShortestPathFinder:Path output port. However, if you want to see the **actual** output, you will have to drag and drop the .gpx file onto Visual Preview. You can then specify to read the data with an LL83 coordinate system, as that information is not stored in GPX files:
