...
Within a fault tolerant system there are two kinds of engine failovers that can occur. The **first** is where the engines are installed along side the core. In the event the server host both a core and an engine goes down the remaining core will take resubmit any failed jobs will continue to send jobs to the available engines. The number of engines on the remaining machine will not increase, however. If you want to increase the number of engines on a machine after a failover event you can from the Web UI with the assumption that the hardware can handle the additional engines. The **second** scenario has a core with a distributed engine on a separate machine. When a failover occurs in this set up the engine will reconnect the active core and continue processing.    



![](./Images/1.011.FailOver_Before.png)
