...
- **FME Workbench Navigator parameters** — the Navigator panel contains several named sections: Readers and Writers (with individual reader/writer parameters), User Parameters, Workspace Parameters (e.g., Rejected Feature Handling), and others. These are distinct, specific UI locations. Apply two rules:
  - **Specific-parameter issue** (the Jira issue names a specific parameter, section, or setting — e.g., "Workspace Parameters > Rejected Feature Handling", a specific reader connection parameter, or a specific User Parameter dialog): the lesson is only relevant if that **exact parameter or section name** is explicitly present in the lesson text. A lesson that sends users to Readers and Writers in the Navigator is NOT relevant to a Workspace Parameters issue. A lesson that mentions the Navigator in general is NOT relevant to a specific parameter issue unless that parameter is named.
  - **Structural/global Navigator issue** (the Jira issue changes the Navigator's overall layout — e.g., the tree structure is reorganized, section icons change, expand/collapse defaults change): the lesson IS relevant if it has students open or interact with the Navigator at any section, because visual/structural changes affect any lesson that shows the Navigator.
...
- A change to a specific Navigator parameter (e.g., "Workspace Parameters > Rejected Feature Handling") should NOT flag a lesson just because it uses the Navigator generally, or because it sets reader/writer parameters — the specific parameter name must appear in the lesson.
- A change that reorganizes the Navigator tree structure or changes Navigator icons SHOULD flag any lesson that has students open or navigate the Navigator, regardless of which section the lesson uses.
