...
- **Version string updates:** Search the entire lesson HTML for every occurrence of `{{FROM_VERSION}}` in text content. For each occurrence found, generate a `change` edit replacing `{{FROM_VERSION}}` with `{{TO_VERSION}}`. Do not skip any occurrence — generate one entry per occurrence. **Exception — "New for FME X.Y" notes:** if the version number appears immediately after the literal text `New for FME ` (with any markup between or around the words — e.g. `<strong>⭐ New for FME 2025.0:</strong>`, or `<p>New for FME <strong>2025.0</strong></p>`), **skip it**. Those are historical markers that record when a feature was introduced and must never be retroactively bumped — a feature that was new in 2025.0 stays new in 2025.0 forever. **Important — FME 2026 release scheme:** Starting with 2026, FME uses a quarterly release model with versions 2026.1, 2026.2, 2026.3, and 2026.4. There is no 2026.0. If the target version is 2026.x or later, never write or suggest `2026.0` as a version string — use the actual target version (e.g. `{{TO_VERSION}}`) directly.
